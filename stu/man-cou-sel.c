...
int cgiMain()
{
	...
	char cid[15]="\0";
	int status=0;
	status = cgiFormString("cid",cid, 15);
  if (status != cgiFormSuccess)
  {
	...
	if(cid[0]=='*'){
sprintf(sql, "select y.*,x.statu 状态 from courseinfo y,course x where y.课程编号=x.cid");
}else{
sprintf(sql, "select y.*,x.statu 状态 from courseinfo y,course x where y.课程编号=x.cid and x.cid='%s'",cid);
}
	//初始化
	db = mysql_init(NULL);
	if (db == NULL)
