...
int cgiMain()
{
  ...
  if (status != cgiFormSuccess)
  {
	...
	if(stuId[0]=='*'){
sprintf(sql, "select y.*,x.statu 状态 from student y,information x where y.学号=x.id");
}else{
sprintf(sql, "select y.* from student y,information x where y.学号=x.id and x.statu=0 and x.id='%s'",stuId);
}
	//初始化
	db = mysql_init(NULL);
	if (db == NULL)
