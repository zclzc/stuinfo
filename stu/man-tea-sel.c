...
int cgiMain()
{
  ...
  if (status != cgiFormSuccess)
  {
	...
	if(tid[0]=='*'){
sprintf(sql, "select tid 教师编号,tname 教师姓名,rank 职称,scname 所属学院,statu 状态 from teacher,school where teacher.scid=school.scid");
}else{
sprintf(sql, "select tid 教师编号,tname 教师姓名,rank 职称,scname 所属学院,statu 状态 from teacher,school where teacher.scid=school.scid and teacher.tid='%s'",tid);
}
	//初始化
	db = mysql_init(NULL);
	if (db == NULL)
 ...
 mysql_query(db, "set character set utf8");
	...
	fprintf(cgiOut, "<div class=\"container\"> <h2 class=\"text-center\">教师信息</h2>");
